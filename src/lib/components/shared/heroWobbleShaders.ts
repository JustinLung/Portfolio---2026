export const wobbleVertexShader = /* glsl */ `
	uniform float uTime;
	...
	void main() {
		...
		float shift = 0.025;
		vec3 positionA = csm_Position + localTangent * shift;
		vec3 positionB = csm_Position + biTangent * shift;

		float wobble = getWobble(csm_Position);
		csm_Position += wobble * normal;
		positionA += getWobble(positionA) * normalize(positionA);
		positionB += getWobble(positionB) * normalize(positionB);
...
export const wobbleFragmentShader = /* glsl */ `
